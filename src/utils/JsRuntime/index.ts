...
export default class JsRuntime
{
    ...
    static assert<E extends Error>( condition: boolean, errorMessage: string | E , addInfos?: any ,...args: any[])
    {
        if( condition ) return;
        ...
        throw (new Error( errorMessage ) as E)
    }

    static throw<E extends Error>( errorMessage: string | E , addInfos?: any ,...args: any[])
    {
        !_JsRuntime_silent && args.length > 0 && console.error(...args);
        !_JsRuntime_silent && addInfos && console.error(addInfos);
